...
def fixed_remask(g, rep, masked_nodes):
    rep[masked_nodes] = 0
    return rep
...
class PreModel(nn.Module):
    def __init__(
        ...
        self._lam = lam
        self._do_feat_encoder = do_feat_encoder
        self._nonzero_mask = nonzero_mask
        ...
        self.feat_linear_encoder = nn.Linear(1, num_hidden)
    ...
    def print_num_parameters(self):
        ...
        print(
            f"num_encoder_params: {sum(num_encoder_params)}, "
            f"num_decoder_params: {sum(num_decoder_params)}, "
            f"num_params_in_total: {sum(num_params)}"
        )
    ...
    def feat_encoder(self, feat):
        if not self._do_feat_encoder:
            return feat
        return self.feat_linear_encoder(feat)
    ...
    def mask_attr_prediction(self, g, x, targets, epoch, drop_g1=None, drop_g2=None):
        ...
        enc_rep = self.encoder(use_g, use_x, )  # [num_nodes, hidden_size]
        ...
        if targets is not None:
            ...
            loss_latent = sce_loss(latent_pred, latent_target, 1)  # 对比mask和不mask的embedding loss
        else:
        ...
        if self._remask_method == "random":
            for i in range(self._num_remasking):
                ...
                elif self._decoder_type in ("gat", "tsgat"):
            ...
            rep = fixed_remask(g, origin_rep, mask_nodes)
    ...
    def encoding_mask_noise(self, g, x, mask_rate=0.3):
        if self._nonzero_mask:
            perm = x.nonzero(as_tuple=True)[0]
            num_nodes = perm.shape[0]
        else:
            num_nodes = g.num_nodes()
            perm = torch.randperm(num_nodes, device=x.device)  # 随机生成一个num_nodes的乱序idx
