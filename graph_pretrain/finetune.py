...
def linear_probing_minibatch(
    ...
    # if dataset_name in ["ogbn-papers100M", "mag-scholar-f", "mag-scholar-c", "ogbn-arxiv", "ogbn-products"]:
    eval_loader = setup_eval_dataloder("lc", graph, feats,
                                       train_ego_graph_nodes + val_ego_graph_nodes + test_ego_graph_nodes, 512)
    ...
    train_emb, val_emb, test_emb = embeddings[:num_train], embeddings[num_train:num_train + num_val], embeddings[
                                                                                                      num_train + num_val:]
...
def node_classification_linear_probing(embeddings, labels, lr, weight_decay, max_epoch, device, mute=False,
    ...
    if batch_size > 0:
        train_loader = LinearProbingDataLoader(np.arange(len(train_emb)), train_emb, train_label, batch_size=batch_size,
                                               num_workers=4, persistent_workers=True, shuffle=shuffle)
        # train_loader = DataLoader(np.arange(len(train_emb)), batch_size=batch_size, shuffle=False)
        val_loader = LinearProbingDataLoader(np.arange(len(val_emb)), val_emb, val_label, batch_size=batch_size,
                                             num_workers=4, persistent_workers=True, shuffle=False)
        test_loader = LinearProbingDataLoader(np.arange(len(test_emb)), test_emb, test_label, batch_size=batch_size,
                                              num_workers=4, persistent_workers=True, shuffle=False)
    else:
...
def finetune(
    ...
    train_loader = setup_finetune_dataloder("lc", graph, feats, train_egs, train_lbls, batch_size=batch_size,
                                            shuffle=True)
    val_loader = setup_finetune_dataloder("lc", graph, feats, val_egs, val_lbls, batch_size=batch_size, shuffle=False)
    test_loader = setup_finetune_dataloder("lc", graph, feats, test_egs, test_lbls, batch_size=batch_size,
                                           shuffle=False)
    ...
    if use_scheduler and max_epoch_f > 0:
        ...
        scheduler = lambda epoch: epoch / warmup_epochs if epoch < warmup_epochs else (1 + np.cos(
            (epoch - warmup_epochs) * np.pi / (max_epoch_f - warmup_epochs))) * 0.5
...
def linear_probing_full_batch(model, graph, x, num_classes, lr_f, weight_decay_f, max_epoch_f, device, linear_prob=True,
                              mute=False):
    model.eval()
    with torch.no_grad():
